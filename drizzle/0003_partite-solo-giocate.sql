-- Verifica 7.5 sul campo (Vitali: 28 presenze a referto, 24 giocate):
-- il tabellino include chi era a referto senza entrare (0 minuti).
-- "partite" e le medie contano solo le partite effettivamente giocate;
-- la riga a referto resta in player_match_stats (serve ai votabili).
create or replace view v_player_season_stats as
select pms.player_id,
       c.season_year,
       c.type_code,
       c.id as competition_id,
       count(*) filter (where pms.minutes > 0)::int as partite,
       ...
       coalesce(avg(pms.oer) filter (where pms.minutes > 0), 0)::numeric(6,4) as oer,
